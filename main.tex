...
\newcommand{\var}{변수(\textit{variable})}
%
...
\title{Calculus for Middle School Students\thanks{Thanks to Edward R.~Tufte for his inspiration.}} % Title of the book

\author[The Tufte-LaTeX Developers]{Chang Huhn,Lee}
...
For our children who shall be the candles in the dark.
\end{doublespace}
\vfill
\vfill
...
\cleardoublepage
\chapter*{소개}% The asterisk leaves out this chapter from the table of contents
...
이 책은 올해 중학교 1학년이 된 아들 `이우원'과 딸 `이지원'이 앞으로 살아가게 될 이 세상이 작동하는 방식에 대해  오랜 세월동안 수많은 사람들의 삶을 통해 
그들의 머리 속에서  건져낸 경험과 사유의 결과인  미분과 적분에 대해 
중학교/고등학교 수준에서 이해할 수 있는 거의 모든 수학적 지식과 기술들을 망라하여 집대성하고자 하였다.
...
\mainmatter
%
%CHAPTER 0 : 사전에 알아야 할 것드
%
\chapter{미분하기 전에 알아야 할 것들}
%미분은 함수라는 개념에 기반을 두고 있다.
수학은 바벨탑처럼 아래에서부터 차곡차곡 쌓아 올려서 만드는 학문이다.
수학의 개념은 반드시 그 기반을 이루는 이전의 개념들 위에 올려쌓은 학문이다.
곱하기는 더하기 개념으로부터 확장된 개념이고, 나누기는 곱하기 개념에서 출발하여 확장된 개념이다.
곱하기나 나누기를 이해하기 위해서는 더하기와 빼기의 개념이 무엇인지 먼저 알아야만 하는 이치와 같은 것이다.

그러므로 미분과 적분의 개념을 본격적으로 알아보기 전에, 이미 알고 있어야 할 개념들이 무엇을 의미하는지 되짚어보도록 하자.

\begin{figure}[h]
\begin{center}
\includegraphics[width=0.8\linewidth]{diagram/diff_couse_graph.png}
\end{center}
\caption{미분 개념을 이해하기 위해 필요한 개념들의 그래프}
\end{figure}
%----------------------------------------------------------------------------------------
%	CHAPTER 1
%----------------------------------------------------------------------------------------
\section{나눗셈}
우리는 초등학생 시절에 나눗셈이 무엇이며, 어떻게 계산하는지 배웠다.
하지만 나누기 연산의 깊은 의미를 제대로 이해하고 있는 학생은 찾아보기 어렵다.
그저 풀이 방법에 맞추어 얼마나 빠른 시간내에  주어진 문제를 정확히 맞추는데  급급해서
그것이 내포한 개념적인 깊은 뜻을 제대로 이해하지 못하고 그동안 지내왔던 게 사실이다.

이 장에서는 나눗셈 연산이 무엇을 의미하는지를 개념적인 관점에서 되짚어 보고, 
나눗셈 연산의 의미를 다양하게 해석하는 방법에 대해 알아보도록 하자.
\subsection{똑같이 나누기}
	\begin{problem}
		사과가 6개 있다. 이것을 2명이 같은 갯수로 나누어 가지려면 한 사람이 사과를 몇개씩 가져가면 되는가?
	\end{problem}
	
	
	한 사람이 나누어 갖게 되는 개수를 $x$라고 하면, (곱하기 연산이 무엇을 의미하는 건지 안다는 가정하에 ) 
	전체 사과의 개수 6개는 2명이 나누어 가진 $x$개의 사과를 2번 더했을 때와 같은 수가 되어야 한다.
	이 문제를 다음과 같이 나타낼 수 있다.
	\footnote{
	사실, 뺄셈도 짝을 이루는 덧셈 방정식을 가지고 있다.
	`사과 6개에서 2개를 빼면 얼마가 되는가?' 와 같은 뺄셈 문제를는  `사과 2개에 몇개를 더하면 6개가 될 것인가?' 와 같은 
	짝을 이루는 덧셈 방정식으로 나타낼 수 있다.
	}
	\begin{equation}
	x+x=2\cdot x=6
	\end{equation}
	나눗셈 연산은  곱셈 방정식의 해를 구하는 문제와 짝을 이루며, 나눗셈 연산의 결과는
	짝을 이루는 곱셈 방정식의 해가 된다.
\subsection{비율}
	\begin{problem}
		밑변이 3이고 높이가 4인 직각 삼각형이 있다.
		이 삼각형과 닮은 삼각형의 밑변의 길이가 1 이면 높이는 얼마일까?
	\end{problem}	
	\begin{figure}[h]
	\begin{center}
		\begin{tikzpicture}
		\draw (0,0)--(1,0)--(1,4/3)--cycle ;
		\draw (0.5,0) node[below]{1};
		\draw (1,2/3) node[right]{$h$};
		\begin{scope}[cm={1,0,0,1,(3,0)}]
					\draw (0,0) --(3,0)--(3,4)-- cycle;
					\draw (1.5,0) node[below]{$1\cdot 3=3$};
					\draw (3,2) node[right]{$h\cdot 3=4$};
		\end{scope}
		\draw [->] (1.5,1)--(3,1);
		\end{tikzpicture}
	\end{center}
	\end{figure}
	위 삼각형과 닮았지만 밑변의 길이가 1인 직각 삼각형(높이는 아직 알 수 없지만 $h$라 두자)을 생각해 보자.
	이 두 삼각형이 닮았다는 것은 작은 삼각형(밑변의 길이가 1)을 가로, 세로로 똑같은 배율로 늘리면
	큰 삼각형(밑변의 길이가 3)과 같아진다는 것을 의미한다.
	이때 높이를 $h$라고 하면 다음과 같은 식이 성립해야 한다.
		\begin{align*}
			1\cdot 3 &= 3\\
			h\cdot 3&=4 \\
		    h&=\frac{4}{3}
		\end{align*}
	길이가 1인 밑변을 3배 키우고 높이도  3배 키우면  
	밑변은 3이, 높이는 4가 되는 $h$의 값이 얼마인가를 묻는 문제와 같다.
	\footnote{일반적으로 서로 직교하는 두 차원 $x,y$의 길이 $a,b$의 비율 $b/a$의 값은 
	$x$ 차원의 길이를   1이라 두면 $y$차원에서의  길이 1을 $a$배 만큼 확대하면
	$x$차원에서의 단위로 길이가  $b$가 된다.
	}
	
	위  문제를 다르게 해석해 보면, 밑변의 가로축에서의 단위 길이가 1인  자로 측정하면 
	세로 축의 1은 가로축의 1의 몇배가 되는가로도 해석할 수 있다.
	\begin{figure}[h]
	\begin{center}
		\begin{tikzpicture}
		\draw(0,0)--(5,0);
		...
		\draw (0,0.5) node[left]{밑변};
		
		
		\begin{scope}[shift={(0,1.2)}]
		\draw[<->](0,0)--(3,0);
		\draw (1.5,0) node[above]{$1\cdot 3=3$};
		\end{scope}
		
		%3번 반복하면 길이가 4와 같아짐을 확인하기 위한 설명선 긋기
		\begin{scope}[shift={(0,-0.8)}]
		\draw[<->] (0,0)--(4,0);
		\draw (2,0) node[above]{$\frac{4}{3}\cdot 3=4$};
		\end{scope}
		...
		\foreach \x %[evaluate=\x as \i using \x *1.333] 
				in {0,1,...,5}{
			\draw (\x*4/3,1)--(\x*4/3,0);
			}
		\draw [<->](0,0.5)--(1*4/3,0.5)	;
		\draw (0.5*4/3,0.5) node[above]{$1=\frac{4}{3}$};	
		\draw (0,0.5) node[left]{높이};
		\end{scope}
		% x=4에서 정확하게 일치하고 있음을 보여주고 있음.
		\draw [red] (4,0)--(4,-1);
		\draw [red] (0,0)--(0,-1);
		\draw [red] (1,0)--(4/3,-1);
		\end{tikzpicture}
	\end{center}
	\caption{밑변의 길이를 1로 두었을때 높이는 $\frac{4}{3}$에 해당하며,
	밑변 길이가 1인 삼각형의 밑변과 높이 모두를  3배 하면 밑변은 3, 높이는 4인 삼각형이 된다.
	}
	\end{figure}
	
\section{함수}
함수(函數)\footnote{여기서 `함'은 선물함, 보석함과 같은 단어에 사용되는 한자로 상자라는 뜻이다. 
함수는 내가 알고 있는 숫자값을 상자에 넣으면 그안에서 기계가 작동하여
결과 숫자값을 내뱉는 가상의 수학 기계를 비유적으로 표현한 것이다.}
는 어떤 \var 의 값이 다른 변수의 값에 의해 결정되는 관계를 말한다. 
\section{방정식}
\section{극한}
...
\newthought{기울기}는 두 변수(variable)
\begin{figure}[h] %h means here, t : top of this page
\begin{center}
\begin{tikzpicture}
\begin{axis}[
axis lines=left,
...
\draw (axis cs:6,0) |- (axis cs:0,36);
\draw (axis cs:6,36) node[right]{$(a,a^2)$};
; % rectangular lines  
\addplot[color=black,
domain=0:10,
]
%expression
{x^2};
\addplot[color=red,domain=3:10,]
{12*x-36};

\end{axis}
\end{tikzpicture}
\end{center}
\caption{$y=x^2$ 함수에서 임의의 점 $(a,a^2)$ 에서의 기울기를 어떻게 계산할까?}
\end{figure}
...
\end{table}
%
% CHAPTER : 적분 전에 알아야 할 것들
%
\chapter{적분 하기 전에 알아야 할 것들}
